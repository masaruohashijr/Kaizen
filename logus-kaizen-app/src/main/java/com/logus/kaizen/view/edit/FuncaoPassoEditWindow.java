...
public class FuncaoPassoEditWindow<T extends Object> extends Dialog {
	 ...
	 */
	private Collection<FuncaoPassoForm> forms;

	private FuncaoPassoForm form;
	 ...
	 */
	public FuncaoPassoItem getObject() {
	 ...
	 */
	public FuncaoPassoEditWindow(String caption, FuncaoPassoForm form, DialogButtonType confirmButtonType,
			boolean readOnly) {
	 ...
	 */
	public FuncaoPassoEditWindow(String caption, FuncaoPassoForm funcaoPassoForm, DialogButtonType confirmButtonType,
	...
	private void confirmAction() {
		try {
			if (!readOnly) {
				...
				BeanGrid<Funcao> gridFuncoesSelector = form.getFuncoesSelector().getGrid();
				Set<Passo> selectedPassos = gridPassosSelector.getSelectedItems();
				Set<Funcao> selectedFuncoes = gridFuncoesSelector.getSelectedItems();
				Object[] arrayPassos = selectedPassos.toArray();
				Object[] arrayFuncoes = selectedFuncoes.toArray();
				FuncaoPassoItem clone = null;
				for (int i = 0; i < arrayPassos.length; i++) {
					for (int j = 0; j < arrayFuncoes.length; j++) {
						clone = new FuncaoPassoItem();
						clone.assignFrom(form.getObject());
						Passo objPasso = (Passo) arrayPassos[i];
						Funcao objPapel = (Funcao) arrayFuncoes[j];
						clone.setPasso(objPasso);
						clone.setFuncao(objPapel);
						validate(clone);
						confirm(clone);
					}
				}
	...
	private void validate(FuncaoPassoItem funcaoPassoItem) {
		PersistenceException ex = new PersistenceException();
		// Validações de anotações
		try {
			JpaValidator.validate(funcaoPassoItem);
	 ...
	 *
	 * @param funcaoPassoItem objeto que será persistido.
	 * @throws Exception qualquer problema ocorrido durante a confirmação.
	 */
	public void confirm(FuncaoPassoItem funcaoPassoItem) throws Exception {
	 ...
	 *                        cancelada.
	 */
	public void cancel(FuncaoPassoItem funcaoPassoItem) {
		// opcional
	}
