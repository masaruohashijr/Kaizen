...
import com.logus.kaizen.view.apoio.projeto.PapelPassoGrid;
import com.logus.kaizen.view.edit.PapelPassoEditWindow;
...
public abstract class PapelPassoListEditor<T extends Object> extends VerticalLayout {

	/**
	 * Objeto que apresentará a listagem.
	 */
	private BeanGrid<PapelPassoItem> grid;
	 ...
	 * @return {@link #grid}.
	 */
	public BeanGrid<PapelPassoItem> getGrid() {
		return grid;
	}
	 ...
	 */
	public PapelPassoListEditor(DialogButtonType confirmButtonType) {
		super();
		setPadding(false);
		setSpacing(true);
	 ...
	 */
	private BeanGrid<PapelPassoItem> internalCreateGrid() {
		grid = createGrid();
	 ...
	 */
	public PapelPassoItem getSelectedObject() {
		return (PapelPassoItem) grid.getSelectedRow();
	}
	...
	protected void setFilter(String filterString) {
		...
		if (filterString != null && !filterString.isEmpty()) {
			grid.getDataProvider().addFilter(item -> {
				for (Column<PapelPassoItem> column : grid.getColumns()) {
	 ...
	 */
	protected PapelPassoItem copyOf(PapelPassoItem object) {
		if (object.getClass().isAnnotationPresent(Entity.class)) {
			PapelPassoItem copy = clone(object);
	 ...
	 */
	public void updateObjects(Collection<PapelPassoItem> objects) {
	 ...
	 */
	public void addObject(PapelPassoItem newObject) {
	 ...
	 * @param object
	 */
	public void removeObject(PapelPassoItem object) {
	 ...
	 */
	public void updateSelecteObject(PapelPassoItem newObject) {
	 ...
	 */
	protected void view() {
		PapelPassoItem selected = getSelectedObject();
		selected = clone(selected);
		Dialog window = new PapelPassoEditWindow<PapelPassoItem>(
	...
	protected void edit() {
		PapelPassoItem selected = getSelectedObject();
		selected = clone(selected);
		final Dialog window = new PapelPassoEditWindow<PapelPassoItem>(
			...
			public void confirm(PapelPassoItem obj) throws Exception {
				updateObject(obj);
	 ...
	 *                fechado.
	 */
	private void insert(PapelPassoItem object, String caption, final Focusable<?> toFocus) {
		final Dialog window = new PapelPassoEditWindow<PapelPassoItem>(caption, createForm(object),
			...
			public void confirm(PapelPassoItem obj) throws Exception {
				if (!objExisteGrid(obj)) {
					...
					// I18N
					NotificationDialog.showError("Papel:" + obj.getPapel() + " e Passo:" + obj.getPasso() + " já foram selecionados. Para alterar a Configuração do Processo, você deve selecionar o Item já cadastrado e acionar o botão 'Alterar'.");
					return;
				}
			}

			private boolean objExisteGrid(PapelPassoItem obj) {
				boolean existe = false;
				String chaveObj = Long.toString(obj.getPasso().getId()) + obj.getPapel();
				List<PapelPassoItem> papeisPassoItens = grid.getItems();
				for (PapelPassoItem papelPassoItem : papeisPassoItens) {
					String chaveItem = Long.toString(papelPassoItem.getPasso().getId()) + papelPassoItem.getPapel();
	 ...
	 *         cópia original.
	 */
	protected PapelPassoItem clone(PapelPassoItem object) {
		if (object instanceof Assignable<?>) {
			try {
				PapelPassoItem clone = (PapelPassoItem) object.getClass().newInstance();
	 ...
	 */
	protected abstract PapelPassoGrid createGrid();
	 ...
	 */
	protected abstract PapelPassoForm createForm(PapelPassoItem object);
	 ...
	 * @return o objeto criado.
	 */
	protected PapelPassoItem createObject() {
		try {
			return (PapelPassoItem) newInstance(0);
	 ...
	 */
	protected abstract void deleteObject(PapelPassoItem obj) throws Exception;
	 ...
	 */
	protected abstract void insertObject(PapelPassoItem obj) throws Exception;
	 ...
	 * @throws Exception caso ocorra algum problema ao atualizar o objeto.
	 */
	protected abstract void updateObject(PapelPassoItem obj) throws Exception;
